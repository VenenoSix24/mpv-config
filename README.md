...
自用 mpv-lazy（hooke007/mpv_PlayKit）的配置管理仓库。
...
- `升级.bat` — 一键升级辅助（详见下文流程第 3 步），生成的上游变更报告放在 `upgrade_reports/`。

## 升级到新版 mpv-lazy 的流程

1. 在新目录全新安装新版 mpv-lazy。
2. 双击运行 `升级.bat`，窗口选择旧版/新版安装目录。脚本会自动完成：
   - 从旧版迁移 `_cache`（watch_later / shader / icc 等运行数据）到新版；
   - 把新版原版 `portable_config` 快照到 `upstream/v<日期>_orig_portable_config`（部署前自动留存，无需手动复制）；
   - 对比新旧两版安装包自带的 `portable_config`，把上游变更报告写到 `upgrade_reports/`；
   - 把仓库 `config/` 部署到新版目录。
3. 打开报告审阅上游变更，需要合并的内容改到仓库 `config/` 里，再跑一次 `sync_to_mpv.bat` 或重跑 `升级.bat`。
4. 在 B 目录验证没问题后删除旧安装目录，运行 `sync_from_mpv.bat` 回收最终状态，git commit。
